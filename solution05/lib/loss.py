...
def photometric_loss(image_1, image_2, pred_flow, weight=None, alpha=0.5, eps=1e-3):
    ...
    # 3. Compute the per-pixel photometric error using the Charbonnier function.
    # Mask where warping was invalid.
    # Weight per-pixel error with the given weight if it is not None.
    photo_error = torch.abs(image_1 - image_2_warped)
    pointwise_photo_loss = charbonnier_loss(photo_error, alpha=alpha, eps=eps) * warping_mask
...
def smoothness_loss(pred_flow, weight=None, alpha=0.3, eps=1e-3):
    ...
    shifteds, shift_masks = shift_multi(pred_flow, offsets)  # NS2HW, NSHW
    smoothness_errors = pred_flow.unsqueeze(1) - shifteds  # NS2HW
    ...
    pointwise_smoothness_loss = torch.sum(
            torch.sum(charbonnier_loss(smoothness_errors, alpha=alpha, eps=eps), dim=2), dim=1,
            keepdim=True)  # N1HW
    # a position in the pointwise loss is valid only if both shifts are valid
    # so here if the returned shift_masks sum up to 2 it means both the shifts are valid
