...
def depth_read(filename):
    ...
    size = width * height
    assert width > 0 and height > 0 and 1 < size < 100000000, ' depth_read:: Wrong input size (width = {0}, height = {1}).'.format(
        width, height)
...
def flow_read(filename):
    ...
    height = np.fromfile(f, dtype=np.int32, count=1)[0]
    size = width * height
    assert width > 0 and height > 0 and 1 < size < 100000000, ' flow_read:: Wrong input size (width = {0}, height = {1}).'.format(
            width, height)
...
class SintelSequence:
    def __init__(self, root, name, renderpass='clean'):
        ...
        self.images = [SintelImage(osp.join(renderpass, name, "frame_{}.png".format(frame))) for
                       frame in frames]
        self.depths = [SintelDepth(osp.join("depth", name, "frame_{}.dpt".format(frame))) for frame
                       in frames]
        self.flows = [SintelFlow(osp.join("flow", name, "frame_{}.flo".format(frame))) for frame in
                      frames]
        self.poses = [SintelPose(osp.join("camdata_left", name, "frame_{}.cam".format(frame))) for
                      frame in frames]
        self.intrinsics = [
                SintelIntrinsics(osp.join("camdata_left", name, "frame_{}.cam".format(frame))) for
                frame in frames]
    ...
    def load(self, root):
        ...
        for key, val in self.data.items():
            if not isinstance(val, list):
                ...
                out_dict[key] = [ele if isinstance(ele, np.ndarray) else ele.load(root) for ele in
                                 val]
...
class Sintel(Dataset):
    def __init__(self, sample_confs, type_, renderpass='clean', sequences=None,
    ...
    def _init_samples(self, sample_confs=None, sequences=None):
        ...
        if sample_list_path is not None and osp.isfile(self._get_sample_list_path()):
            ...
            self._init_samples_from_confs(sample_confs=sample_confs, sequences=sequences)
            self.write_samples()
    ...
    def _init_samples_from_confs(self, sample_confs, sequences=None):
        ...
        sequence_names = [s for s in sequence_names if
                          s in sequences] if sequences is not None else sequence_names
