...
def _get_marker_range_text(markers, marker_cmap):
    ...
    if markers:  # not None and not empty
        ...
        with np.errstate(divide='ignore', invalid='ignore'):
            min_marker_score_text = np.nanmin([marker['score'] for marker in markers]
                                              ) if use_marker_range else None
            max_marker_score_text = np.nanmax([marker['score'] for marker in markers]
                                              ) if use_marker_range else None
        is_marker_score_constant = (min_marker_score_text == max_marker_score_text
                                    if use_marker_range else None)
