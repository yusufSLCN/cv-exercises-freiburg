...
class FlyingChairs(Dataset):
    ...
    def _init_samples(self, split=None):
        ...
        for i in range(len(flows)):
            xid = split_list[i]
            if (split is None) or (split == 'training' and xid == 1) or (
                    split == 'validation' and xid == 2):
                sample = FlyingChairsSample(
                        name=flows[i][:5], images=[images[2 * i], images[2 * i + 1]], flow=flows[i])
...
class FlyingChairsTrain(FlyingChairs):
    ...
    def __init__(self, root=None, aug_fcts=None, to_torch=False, verbose=True):
        split = "training"
        super().__init__(split=split, root=root, aug_fcts=aug_fcts, to_torch=to_torch,
                         verbose=verbose)
