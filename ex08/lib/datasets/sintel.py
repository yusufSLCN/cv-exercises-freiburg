...
def depth_read(filename):
    ...
    assert check == TAG_FLOAT, (' depth_read:: Wrong tag in flow file (should be: {0}, is: {1}). '
                                'Big-endian machine? '.format(TAG_FLOAT, check))
...
def flow_read(filename):
    ...
    f = open(filename, 'rb')
    check = np.fromfile(f, dtype=np.float32, count=1)[0]
    assert check == TAG_FLOAT, (' flow_read:: Wrong tag in flow file (should be: {0}, is: {1}). '
                                'Big-endian machine? ').format(TAG_FLOAT, check)
    width = np.fromfile(f, dtype=np.int32, count=1)[0]
    height = np.fromfile(f, dtype=np.int32, count=1)[0]
    size = width * height
...
def cam_read(filename):
    ...
    f = open(filename, 'rb')
    check = np.fromfile(f, dtype=np.float32, count=1)[0]
    assert check == TAG_FLOAT, (' cam_read:: Wrong tag in flow file (should be: {0}, is: {1}). '
                                'Big-endian machine? '.format(TAG_FLOAT, check))
...
class SintelSequence:
    def __init__(self, root, name, renderpass='clean'):
        ...
        self.images = [SintelImage(osp.join(renderpass, name, "frame_{}.png".format(
                frame))) for frame in frames]
        self.depths = [SintelDepth(osp.join("depth", name, "frame_{}.dpt".format(
                frame))) for frame in frames]
        self.flows = [SintelFlow(osp.join("flow", name, "frame_{}.flo".format(
                frame))) for frame in frames]
        self.poses = [SintelPose(osp.join("camdata_left", name, "frame_{}.cam".format(
                frame))) for frame in frames]
        self.intrinsics = [SintelIntrinsics(osp.join("camdata_left", name, "frame_{}.cam".format(
                frame))) for frame in frames]
    ...
    def load(self, root):
        ...
        for key, val in self.data.items():
            if not isinstance(val, list):
                ...
                out_dict[key] = [ele if isinstance(ele, np.ndarray) else ele.load(root)
                                 for ele in val]
...
class Sintel(Dataset):
    ...
    def _init_samples(self, sample_confs, sequences=None):
        ...
        sequence_names = [Path(s).as_posix() for s in sequence_names]
        sequence_names = [s for s in sequence_names if s in sequences
                          ] if sequences is not None else sequence_names
                    ...
                    for key, conf in sample_conf.items():
                        if isinstance(conf, tuple):
                            ...
                            for conf_ in conf:
                                ...
                                if offset_num in sequence.id_to_idx:
                                    offset_idx = sequence.id_to_idx[offset_num]
                                    sample.data[key].append(getattr(
                                            sequence, data_type)[offset_idx])
                                else:
                                    sample_valid = False
                                    break
