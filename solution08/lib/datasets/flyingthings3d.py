...
class FlyingThings3D(Dataset):
    def __init__(self, sample_confs, split, use_subset=False, root=None, aug_fcts=None,
                 to_torch=False, verbose=True):
        self.split = split
                    ...
                    if isinstance(conf, DataConf):
                        ...
                        for conf_idx, conf_ in enumerate(conf):
                            ...
                            for file in os.listdir(osp.join(sequence, conf_.path)):
                                ...
                                for filtered_sample in filtered_samples:
                                    if (filtered_sample[0] in sequence
                                            and filtered_sample[1] in file):
                                        exclude_file = True

                                if not exclude_file:
                                    files[key][conf_idx][frame_num] = osp.join(conf_.path, file)
                                    frame_nums.add(frame_num)
