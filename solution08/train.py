...
def train(args):
    ...
    optimizer, scheduler = setup_optimization(model=model, finished_iterations=finished_iterations,
                                              args=args)
    saver_all, saver_weights_only = setup_savers(model=model, optimizer=optimizer,
                                                 scheduler=scheduler,
    ...
    while finished_iterations < max_iteration:
        ...
        for iter_in_epoch, sample in enumerate(dataloader):
            ...
            if finished_iterations % LOG_INTERVAL == 0:
                logger.log(sample=sample, model_output=model_output, loss=loss,
...
def setup_dataloader(args):
    ...
    dataloader = dataset_cls.init_as_loader(
            ...
            pin_memory=True, drop_last=True, aug_fcts=aug_fct)
    return dataloader
...
def setup_optimization(model, finished_iterations, args):
    ...
    scheduler = torch.optim.lr_scheduler.MultiStepLR(optimizer, milestones=lr_intervals,
                                                     gamma=gamma)
