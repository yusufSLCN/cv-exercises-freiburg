...
def main():
    ...
    parser.add_argument('--output',
                        help="Path to folder for output data like logs and model weights.")
    parser.add_argument('--iterations', default=600000, type=int,
                        help="Number of training iterations.")
    parser.add_argument('--model', default="FlowNetC", help="FlowNetC or FlowNetS model.")
    parser.add_argument('--completed_iterations', type=int,
                        help="Already completed number of iterations.")
    parser.add_argument('--restore', help="Path to a checkpoint to restore from.")
    parser.add_argument('--auto_restore', help="Restore checkpoint using path from "
                                               "models.toml file.", type=str, default=None)
    ...
    parser.add_argument('--lr_schedule', default="short",
    ...
    parser.add_argument('--smoothness_loss', action='store_true',
...
def train(args):
    ...
    saver_all, saver_weights_only = setup_savers(
            model=model, optimizer=optimizer, scheduler=scheduler, checkpoint_dir=checkpoint_dir)

    if args.restore is not None:
        restore(saver_all=saver_all, saver_weights_only=saver_weights_only,
    ...
    while finished_iterations < max_iteration:
        ...
        for iter_in_epoch, sample in enumerate(dataloader):
            ...
            if finished_iterations % PRINT_INTERVAL == 0:
                ...
                print("Iteration {}/{} - {:.2f}s per iteration - loss: {:.5f}".format(
                        finished_iterations,
                        max_iteration,
            ...
            if finished_iterations % LOG_INTERVAL == 0:
                logger.log(
                        sample=sample, model_output=model_output, loss=loss, sub_losses=sub_losses,
...
def setup_savers(model, optimizer, scheduler, checkpoint_dir):
    saver_all = TrainStateSaver(model=model, optim=optimizer, scheduler=scheduler,
                                base_path=checkpoint_dir,
                                base_name="checkpoint-train", max_to_keep=4)

    saver_weights_only = WeightsOnlySaver(model=model, base_path=checkpoint_dir,
                                          base_name="checkpoint-model")
