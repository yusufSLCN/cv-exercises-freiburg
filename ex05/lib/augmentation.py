...
class UniformBernoulli:
    ...
    def sample_noexp(self, size=1):
        ...
        return gate * np.random.uniform(low=self.mean - self.spread, high=self.mean + self.spread,
                                        size=size)

    def sample_exp(self, size):
        gate = Bernoulli(self.prob).sample(size=1)
        return gate * np.exp(
                np.random.uniform(low=self.mean - self.spread, high=self.mean + self.spread,
                                  size=size))
...
class FlowNetAugmentation:
    def __init__(self):
        ...
        self.color_aug = ColorJitter(saturation=(0, 2), contrast=(0.01, 8), brightness=(0.01, 2.0),
                                     hue=0.5)
    ...
    def __call__(self, sample):
        ...
        gt_disp = sample['gt_disp'] if (
                'gt_disp' in sample and not self.augment_image_only) else None  # H, W
        gt_flow = sample['gt_flow'] if (
                'gt_flow' in sample and not self.augment_image_only) else None  # 2, H, W
        ...
        images_spatial = [np.transpose(image_spatial, [2, 0, 1]) for image_spatial in
                          images_spatial]  # 3, H, W
    ...
    def color_transform(self, images):
        if np.random.rand() < self.color_aug_prob:
            ...
            if np.random.rand() < self.asymmetric_color_aug_prob:
                images = [np.array(self.color_aug(Image.fromarray(image)), dtype=np.uint8) for image
                          in images]
    ...
    def spatial_transform(self, images, flow, disp):
        ...
        if np.random.rand() < self.spatial_aug_prob:
            ...
            if images is not None and len(images) > 0 and any(
                    [image is not None for image in images]):
                images = [image[y0:y0 + cht, x0:x0 + cwd] if image is not None else None for image
                          in images]
            ...
            if images is not None and len(images) > 0 and any(
                    [image is not None for image in images]):
                images = [cv2.resize(image, (cwd, cht),
                                     interpolation=cv2.INTER_LINEAR) if image is not None else None
                          for image in images]
