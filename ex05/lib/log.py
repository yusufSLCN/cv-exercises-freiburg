...
def log_tensor_list(writer, desc, tensor_list, step, labels=None, idx=None, full_batch=False,
                    every_nth=None,
                    colorize=True, clipping=False, upper_clipping_thresh=None,
    ...
    for tensor_idx, tensor in enumerate(tensor_list):
        ...
        label = (idx_label + "/" + str(label) if not isinstance(label,
                                                                str) else idx_label + label) if idx_label is not None else label
...
class Logger:
    ...
    def log_in_data(self, sample, step):
        ...
        log_tensor(self.writer, "{}/{}/00_images/ref".format(self.overview_base, base_name),
                   images[0], step,
                   full_batch=full_batch)
        ...
        if gt_flow is not None:
            ...
            log_tensor(self.writer, "{}/{}/01_gt_flow".format(self.overview_base, base_name),
                       gt_flow, step,
                       full_batch=full_batch)

        if gt_disp is not None:
            log_tensor(self.writer, base_name + "/02_gt_disp", gt_disp, step, full_batch=full_batch)
            log_tensor(self.writer, "{}/{}/02_gt_disp".format(self.overview_base, base_name),
                       gt_disp, step,
                       full_batch=full_batch)
    ...
    def log_model(self, sample, step):
        ...
        for model_vis_name, vis in model_vis.items():
            log_tensor(self.writer, "{}/{}".format(base_name, model_vis_name), vis, step,
                       full_batch=full_batch)
    ...
    def log_pred(self, model_output, step):
        ...
        if pred_flow is not None:
            log_tensor(self.writer, base_name + "/01a_pred_flow", pred_flow, step,
                       full_batch=full_batch)
            log_tensor(self.writer, "{}/{}/01_pred_flow".format(self.overview_base, base_name),
                       pred_flow, step,
                       full_batch=full_batch)
    ...
    def log_loss(self, loss, sub_losses, pointwise_losses, step, scalars_only):
        ...
        log_scalar(self.writer, "{}/{}/0_total_loss".format(self.overview_base, base_name), loss,
                   step)
    ...
    def log_optim(self, step):
        ...
        for i, param_group in enumerate(optimizer.param_groups):
            lr = param_group['lr']
            log_scalar(self.writer, "{}/{}/lr/group_{}".format(self.overview_base, base_name, i),
                       lr, step)
        ...
        for name, param in model.named_parameters():
            ...
            if param.grad is not None:
                log_histogram(self.writer, "{}/1_grads/{}".format(base_name, name), param.grad,
                              step,
                              replace_NaNs=False)
    ...
    def log_metrics(self, metrics, qualitatives, step):
        ...
        for idx, (metric_name, metric_val) in enumerate(metrics.items()):
            log_scalar(self.writer, "{}/{}_{}".format(base_name, idx, metric_name), metric_val,
                       step)
