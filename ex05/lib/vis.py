...
_DEFAULT_FONTS = {_DEFAULT_FONT_SIZE: ImageFont.truetype(_DEFAULT_FONT_PATH,
                                                         _DEFAULT_FONT_SIZE) if os.path.isfile(
...
def _get_default_font(size=None):
    if size is None:
        ...
        return ImageFont.truetype(_DEFAULT_FONT_PATH, size) if os.path.isfile(
                _DEFAULT_FONT_PATH) else None
...
def _get_marker_range_text(markers, marker_cmap):
    ...
    if markers:  # not None and not empty
        ...
        with np.errstate(divide='ignore', invalid='ignore'):
            min_marker_score_text = np.nanmin(
                    [marker['score'] for marker in markers]) if use_marker_range else None
            max_marker_score_text = np.nanmax(
                    [marker['score'] for marker in markers]) if use_marker_range else None
        is_marker_score_constant = min_marker_score_text == max_marker_score_text if use_marker_range else None
        ...
        if is_marker_score_constant:
            ...
            marker_range_text = (f"Markers: Min ({min_marker_color}): {min_marker_score_text:0.3f} "
                                 f"Max ({max_marker_color}): {max_marker_score_text:0.3f}")

    return marker_range_text


def _get_draw_text(text, text_off, image_range_text, image_range_text_off, marker_range_text,
    ...
    if text is not None and not text_off:
        ...
        if (
                marker_range_text is not None and not marker_range_text_off) or not image_range_text_off:
            draw_text += "\n"
...
def _convert_to_out_format(img, out_format):
    if out_format['type'] == 'PIL':
        ...
        out = np.transpose(out, [2, 0, 1]) if 'channels' in out_format and out_format[
            'channels'] == 'CHW' else out
    else:
...
def np2d(arr, colorize=True,
         ...
         markers=None, marker_radius=None, marker_text_off=False, marker_cmap=_DEFAULT_CMAP,
         ignore_marker_scores=False, min_marker_score=None, max_marker_score=None,
         image_range_text_off=False, image_range_colors_off=False,
         marker_range_text_off=False, text_off=False,
    ...
    (arr, invalid_mask, invalid_values_mask, clipping_mask, upper_clipping_mask,
     lower_clipping_mask, upper_clipping_thresh, lower_clipping_thresh) = invalidate_np_array(
    ...
    if markers:  # not None and not empty
        if scale != 1.0:
            ...
            for marker in markers:
                marker['xy_pos'] = (marker['xy_pos'][0] * scale, marker['xy_pos'][1] * scale)
        add_markers_to_img(img=img, markers=markers, marker_radius=marker_radius,
                           marker_text_off=marker_text_off,
                           cmap=marker_cmap, ignore_scores=ignore_marker_scores,
...
def np3d(arr, channels='RGB', text=None, gray=False,
         clipping=False, upper_clipping_thresh=None, lower_clipping_thresh=None,
         mark_clipping=False, clipping_color=None,
         invalid_values=None, mark_invalid=False, invalid_color=None,
         markers=None, marker_radius=None, marker_text_off=False, marker_cmap=_DEFAULT_CMAP,
    ...
    if channels == 'FLOW':
        vis_arr = _visualize_np_flow_array(arr=arr, text=text,
                                           mark_invalid=mark_invalid, invalid_color=invalid_color,
                                           markers=markers, marker_radius=marker_radius,
                                           marker_text_off=marker_text_off, marker_cmap=marker_cmap,
                                           ...
                                           text_off=text_off,
                                           out_format=out_format, out_action=out_action)
        ...
        vis_arr = _visualize_np_rgb_array(arr=arr, text=text, gray=gray,
                                          clipping=clipping,
...
def get_cmap_color(value, lower_thresh, upper_thresh, cmap=_DEFAULT_CMAP, channels='rgb'):
    ...
    norm_value = max(min(((value - lower_thresh) / (upper_thresh - lower_thresh)), 1.0), 0.0)
    if channels == 'rgb':
...
def add_text_to_img(img, text,
    ...
    if do_resize:
        resize_factor = 1.0
        if xy_lefttop is not None:
            while img.width < text_size[0] + xy_lefttop[0] or img.height < text_size[1] +\
                    xy_lefttop[1]:
            ...
            while img.width < text_size[0] + xy_leftbottom[0] or img.height < text_size[1] +\
                    xy_leftbottom[1]:
                if max_resize_factor is not None and resize_factor * 2 > max_resize_factor:
                    break
    ...
    if bbox_color is not None or bbox_stroke is not None:
        ...
        bbox = ([(xy_lefttop[0] - bbox_space, xy_lefttop[1]),
...
def add_markers_to_img(img, markers, marker_radius=None, marker_text_off=False, cmap=_DEFAULT_CMAP,
                       ignore_scores=False,
    ...
    marker_radius = min(img.height / 70, img.width / 70) if marker_radius is None else marker_radius
    ...
    if not ignore_scores:
        ...
        for marker in markers:
            ...
            if 'score' in marker:
                min_score = min(min_score, marker['score']) if do_update_min_score else min_score
                max_score = max(max_score, marker['score']) if do_update_max_score else max_score
            else:
                use_scores = False
                break
    ...
    for marker in markers:
        ...
        if use_scores:
            if np.isfinite(marker['score']):
                marker_color = get_cmap_color(value=marker['score'], lower_thresh=min_score,
        ...
        bbox_color = marker['bbox_color'] if 'bbox_color' in marker else _DEFAULT_BBOX_COLOR
        bbox_stroke = marker['bbox_stroke'] if 'bbox_stroke' in marker else _DEFAULT_BBOX_STROKE

        r = marker_radius if 'marker_radius' not in marker else marker['marker_radius']
        draw.ellipse((max(0, xy_pos[0] - r), max(0, xy_pos[1] - r), xy_pos[0] + r, xy_pos[1] + r),
                     fill=marker_color)

        desc = marker['desc'] if 'desc' in marker else None
        desc_text_off = marker['desc_text_off'] if 'desc_text_off' in marker else False
        score_text_off = marker['score_text_off'] if 'score_text_off' in marker else False

        show_desc_text = desc is not None and not desc_text_off
        show_score_text = 'score' in marker and np.isfinite(marker['score']) and not score_text_off

        if not marker_text_off and (show_desc_text or show_score_text):
            text = (desc if show_desc_text else "") + (
                    "\n" if (show_desc_text and show_score_text) else "") + (
                           "s=" + '%0.2f' % (marker['score']) if show_score_text else "")
            add_text_to_img(img=img, text=text, xy_leftbottom=(xy_pos[0], img.height - xy_pos[1]),
                            x_rel_shift=-0.5,
                            y_rel_shift=-1.0, y_abs_shift=-r - 2, do_resize=False,
                            text_color=text_color, bbox_color=bbox_color, bbox_stroke=bbox_stroke)
...
def _visualize_np_flow_array(arr, text=None,
                             mark_invalid=False, invalid_color=None,
                             markers=None, marker_radius=None, marker_text_off=False,
                             marker_cmap=_DEFAULT_CMAP,
                             ignore_marker_scores=False, min_marker_score=None,
                             max_marker_score=None,
                             image_range_text_off=False, marker_range_text_off=False,
                             text_off=False,
                             out_format=None, out_action=None):
    """
    ...
    arr, invalid_mask, invalid_values_mask, _, _, _, _, _ = invalidate_np_array(
            arr)  # add clipping here if you want
    invalid_flow_values_mask = np.logical_and(invalid_values_mask[:, :, 0],
                                              invalid_values_mask[:, :, 1])
    ...
    for color_channel_idx in range(colorwheel.shape[1]):  # color_channel_idx in [0,1,2] -> r, g, b
    ...
    image_range_text = "Flow: X-Range: (%0.1f, %0.1f) Y-Range: (%0.1f, %0.1f)" % (
            flow_x_min, flow_x_max, flow_y_min, flow_y_max)
    draw_text = _get_draw_text(text, text_off, image_range_text, image_range_text_off,
                               marker_range_text,
                               marker_range_text_off)
...
def _visualize_np_rgb_array(arr, text, gray=False,
                            clipping=False, upper_clipping_thresh=None, lower_clipping_thresh=None,
                            mark_clipping=False, clipping_color=None,
                            invalid_values=None, mark_invalid=False, invalid_color=None,
                            markers=None, marker_radius=None, marker_text_off=False,
...
def invalidate_np_array(arr, clipping=False, upper_clipping_thresh=None, lower_clipping_thresh=None,
                        invalid_values=None):
    """
    ...
    if clipping:
        if upper_clipping_thresh is None or lower_clipping_thresh is None:
            ...
            if upper_clipping_thresh is None:
                upper_clipping_thresh = min(np.nanmax(arr[~invalid_values_mask]),
                                            mean + 2 * std) if not all_values_invalid else np.nan
            if lower_clipping_thresh is None:
                lower_clipping_thresh = max(np.nanmin(arr[~invalid_values_mask]),
                                            mean - 2 * std) if not all_values_invalid else np.nan

        with np.errstate(invalid='ignore'):
            upper_clipping_mask = np.logical_and((arr > upper_clipping_thresh),
                                                 ~invalid_values_mask)
            lower_clipping_mask = np.logical_and((arr < lower_clipping_thresh),
                                                 ~invalid_values_mask)
        clipping_mask = upper_clipping_mask | lower_clipping_mask  # True = value should be clipped
    else:
        clipping_mask = np.zeros_like(arr,
                                      dtype='bool')  # All False because no values should be clipped
