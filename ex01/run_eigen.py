...
from lib.eigendecomp import get_inverse, get_dot_product, get_euclidean_norm, get_matrix_from_eigdec
...
def main():
    ...
    np.testing.assert_allclose([norm1, norm2], [1.0, 1.0], err_msg=f"Norms should be 1 but are {norm1} and {norm2}")

    # calculate dot product of the vectors and check it
    dot = get_dot_product(vec1, vec2)
    np.testing.assert_allclose([dot], [0.0], err_msg=f"Dot product should be 0 but is {dot}")
